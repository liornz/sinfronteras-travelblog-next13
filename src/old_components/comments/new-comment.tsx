...
import { enteredCommentData } from '../../utils/types';
...
const NewComment: React.FC<Props> = (props) => {
  ...
  const invalidInputMsg = t('validate');
    ...
    <form className={styles.form} onSubmit={submitCommentHandler}>
      <div className={styles.row}>
        <div className={styles.control}>
          ...
          <input type="email" id="email" ref={emailInputRef as React.LegacyRef<HTMLInputElement>} />
        </div>
        <div className={styles.control}>
          <label htmlFor="name">{t('name')}</label>
          <input type="text" id="name" ref={nameInputRef as React.LegacyRef<HTMLInputElement>} />
        </div>
      </div>
      <div className={styles.control}>
        <label htmlFor="comment">{t('comment')}</label>
        <textarea name="comment" id="comment" rows={5} ref={commentInputRef as React.LegacyRef<HTMLTextAreaElement>}></textarea>
      </div>
      {isInvalid ? (
